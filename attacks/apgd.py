...
class APGD(Attack):
    ...
    def attack_single_run(self, x_in, y_in):
        ...
        if self.norm == "Linf":
            t = 2 * torch.rand(x.shape).to(self.device, torch.float16).detach() - 1
        ...
        step_size = (
            self.eps
            * torch.ones([x.shape[0], 1, 1, 1]).to(self.device, torch.float16).detach()
            * torch.Tensor([2.0]).to(self.device, torch.float16).detach().reshape([1, 1, 1, 1])
        )  # nopep8
            ...
            with torch.no_grad():
                ...
                if self.norm == "Linf":
                    ...
                    x_adv_1 = torch.clamp(
                        ...
                        * torch.min(
                            self.eps * torch.ones(x.shape).to(self.device, torch.float16).detach(),
                            ((x_adv_1 - x) ** 2)
    ...
    def perturb(self, x_in, y_in, best_loss=False, cheap=True):
        ...
        if not best_loss:
            ...
            loss_best = torch.ones([x.shape[0]]).to(self.device, torch.float16) * (
                -float("inf")
            )  # nopep8
            for counter in range(self.n_restarts):
