...
class Attack(object):
    ...
    def normalize(self, inputs):
        mean = self.normalization_used["mean"].to(inputs.device, torch.float16)
        std = self.normalization_used["std"].to(inputs.device, torch.float16)
        ...
        for step, (inputs, labels) in enumerate(data_loader):
            ...
            if verbose or return_verbose:
                with torch.no_grad():
                    ...
                    delta = (adv_inputs - inputs.to(self.device, torch.float16)).view(
                        batch_size, -1
                    )  # nopep8
                    l2_distance.append(
