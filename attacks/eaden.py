...
class EADEN(Attack):
    ...
    def forward(self, images, labels):
        ...
        images = images.clone().detach().to(self.device, self.dtype)
        ...
        lower_bound = torch.zeros(batch_size, device=self.device, dtype=self.dtype)
        const = torch.ones(batch_size, device=self.device, dtype=self.dtype) * self.initial_const
        upper_bound = torch.ones(batch_size, device=self.device, dtype=self.dtype) * 1e10

        final_adv_images = images.clone()
        y_one_hot = torch.eye(outputs.shape[1]).to(self.device, self.dtype)[labels]

        o_bestl1 = [1e10] * batch_size
        o_bestscore = [-1] * batch_size
        o_bestl1 = torch.Tensor(o_bestl1).to(self.device, self.dtype)
        o_bestscore = torch.Tensor(o_bestscore).to(self.device, self.dtype)
        ...
        for outer_step in range(self.binary_search_steps):
            ...
            bestl1 = torch.Tensor(bestl1).to(self.device, self.dtype)
            bestscore = torch.Tensor(bestscore).to(self.device, self.dtype)
            prevloss = 1e6
    ...
    def FISTA(self, images, x_k, y_k):
        ...
        diff = y_k - images
        cond1 = (diff > self.beta).float().to(self.device, self.dtype)
        cond2 = (torch.abs(diff) <= self.beta).float().to(self.device, self.dtype)
        cond3 = (diff < -self.beta).float().to(self.device, self.dtype)
    ...
    def adjust_best_result(
        ...
        output_label = torch.argmax(output, 1).float().to(self.dtype)
